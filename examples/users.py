...
async def books_by_author_table(
    ...
    return Table(
        data=[
            {"author": f"{a.first_name} {a.last_name}", "book_count": count}
            for a, count in rows
        ]
    )
...
@page.pie_chart(
    "Authors with Bio", "Distribution of authors with and without biographies"
)
async def authors_bio_chart(session: AsyncSessionDep) -> PieChart:
    ...
    without_bio = (
        await session.execute(
            select(func.count()).select_from(Author).where(Author.bio.is_(None))
        )
    ).scalar_one()
    return PieChart(
        data=[
            {"segment": "With Bio", "count": with_bio},
            {"segment": "Without Bio", "count": without_bio},
        ],
